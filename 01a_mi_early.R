...
#paths
path_micro <- "data/micro_early.tsv"
path_genes <- "data/genes_early.tsv"
...
saveRDS(microXgenes, file = "results/early.rds")

# mi_matrix <- bind_rows(!!!microXgenes, #explicit splicing
#                        .id = "micro/gene")
# 
# vroom::vroom_write(x = mi_matrix, path = "results/early.txt")
